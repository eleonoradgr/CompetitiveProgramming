...
pair<bool, pair<int, int>> checkBST(Node *tree) {
    if (tree->right == NULL && tree->left == NULL) { //leaf of tree
        ...
        pair<int, int> minmax = make_pair(min(rsubtree.second.first, tree->data),
                                          max(rsubtree.second.second, tree->data));
        ...
        pair<int, int> minmax = make_pair(min(lsubtree.second.first, tree->data),
                                          max(lsubtree.second.second, tree->data));
        return make_pair(isbst, minmax);
    } else {
        pair<bool, pair<int, int>> rsubtree = checkBST(tree->right);
        pair<bool, pair<int, int>> lsubtree = checkBST(tree->left);
        bool isbst = lsubtree.first && (tree->data > lsubtree.second.second) && rsubtree.first &&
                     (tree->data <= rsubtree.second.first);
